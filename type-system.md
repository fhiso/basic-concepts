...
An **entity** is an identifiable concept which other concepts may reference.
...
Every *class* is therefore a **subclass** of `rdfs:Resource`.
...
The *property* which identifies the *range* of a *property name* uses the *property name* `rdfs:range` and has *range* `rdfs:Class`.
...
A **datatype** is an *term* which is a formal description of how certain *strings* can be mapped directly to certain *entities* such that the identity and meaning of each *entity* can be inferred directly from that *datatype*'s treatment of the *string* without the need for explicit *properties*.
...
{.note}
This definition of a *datatype* is sufficiently aligned with XML Schema's notion of a simple type, as defined in \[[XSD Pt2](https://www.w3.org/TR/xmlschema11-2/)\], that XML Schema's simple types can be used as *datatypes* in this standard. Best practice on how to get an IRI for use as the *term name* of XML Schema types can be found in \[[SWBP XSD DT](https://www.w3.org/TR/swbp-xsch-datatypes/)\]. Similarly, this standard's definition of a *datatype* is very similar to the definition of a datatype in \[[RDF Concepts](https://www.w3.org/TR/rdf11-concepts/)\], and RDF datatypes can be used as *datatypes* in this standard.

{.example}
XML Schema defines an integer type in §3.4.13 of \[[XSD Pt2](https://www.w3.org/TR/xmlschema11-2/)\] which is well-suited for use in this standard. FHISO uses this type where integer values occur. It discussed in {§integer} of this standard.
...
{.example}
The XML Schema `integer` *datatype* used in the previous example is one where the mapping from lexical representation to value is many-to-one rather than one-to-one. This is due to *lexical space* including strings with a leading `+` sign as well as superfluous leading `0`s, and means that "`00137`", "`+137`" and "`137`" all represent the same underlying value: the number one hundred and thirty-seven. Because *conformant* applications *may* convert strings between equivalent lexical representations, they *may* store them in a database in an integer field and regenerate *strings* in a canonical representation.
...
{.example ...}  FHISO intends to standardise a *datatype* with identified by the *term* `types:CreatorsName`

This *datatype* will include both internal formatting to support various name orders and sorting rules, and also includes language tags to represent the human languages in which names are presented.
...
{.note ...}
This document separates concepts from their representations even where such separation may seem redundant to some readers. A few motivations for this separation include:

*  Implementations might wish to use an optimized binary in-memory representation for various concepts internally instead of the serialised representations defined in FHISO standards. Understanding the underlying concepts such optimized representations need to represent may help implementers do this correctly.

*  Some concepts are represented only implicitly in some serialisations; it is useful to have a vocabulary for those implicit components. For example, a formatted citation could be characterized as a set of *formal properties* about an unidentified source *entity* even the source *entity* ha not *term* to describe it.

*  Having this separation streamlines defining a single data model with multiple distinct serialisations.
{/}
...
An **entity class** is an *entity* identifying a particular context or use in which other *entities* may be defined. An *entity* is said to be a **member** of an *entity class* if the context or use of that *entity class* applies to the *entity*.
...
Thus, every *class* is a **subclass** of `rdfs:Resource`.
...
A **property** is a communicable representation of a *formal property* where the *predicate* is represented by a **property name**, which is a *term*; and the *direct object* is represented by a **property value**, which is either a *term* or *literal value*.
...
When a *term* has been defined for use in the context denoted by some *class*, that *class* is referred to as the **type** of the *term*. The *type* of a *term* is a piece of information which *must* be provided, perhaps implicitly, when defining a *term*.  As such, the *type* is a *property* of the *term*, as defined in {§properties}, and needs a *property term* to represent it.  This standard uses the `rdf:type` *term* for this purpose:
...
{.ednote} basic-concepts refers to "the type" of a term, but surely there is more than one such type?


A **value class** is an *entity class*, all *members* of which can be serialised according to a *datatype* such such that the identity and meaning of each *entity* can be inferred directly from that serialisation.

{.note} We expect there to be only a few *value classes* in any given standard: names, dates, numbers, text, and a few related ideas cover the bulk of current family history data models.

A **datatype** is a *term*; its *entity* represents a mapping between a *value class* and a set of *strings*. This mapping *must* map every *member* of the *value class* to a *string* and map distinct *members* of the *value class* to distinct *strings*, but *may* map several *strings* to the same *entity* and *may* have some *strings* that map to no *member* of the *value class*.

{.example} The `xsd:integer` *datatype* can map the *entity* "three" to the *string* `3`, or `03`, or `0003`, or any of an infinite number of other *strings*. No *entity* other than *three* in the integer *value class* maps to any of these strings.
...
{.example ...} Consider the following *literal value* consisting of two *literals*:

|string                 |datatype           |language|
|-----------------------|-------------------|--------|
|`Tychonievich, Luther` |`cev:creatorName`  |`en`    |
|`タイコウノズ, ルテル`    |`cev:creatorName`  |`ja`    |

This example shows a case where literals might have both non-default datatypes and languages.
{/}
